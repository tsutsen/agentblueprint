...
function registerGraphMetrics(pi: ExtensionAPI, extDir: string) {
  const script = path.join(extDir, "scripts", "graph_metrics.py");

  // Helper to run the script
  async function runMetrics(artifacts: string, format: string, reportPath: string | null = null): Promise<{
    content: { type: string; text: string }[];
    details: { success: boolean; output?: string; stderr?: string };
    isError?: boolean;
  }> {
    if (!fs.existsSync(script)) {
      return {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      // Parse the JSON to extract errors
      try {
...
// ── Tool: graph-visualize ────────────────────────────────────────────────────

function registerGraphVisualize(pi: ExtensionAPI, extDir: string) {
